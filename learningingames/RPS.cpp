...
void playGame(const vector<vector<vector<double>>> payoff, double eps1, double eps2, int T, double h)
{
    ...
    for(int t = 1; t <= T; t++)
    {
        ...
        // Update Weight
        for(int i = 0; i < k; i++) w1[i] *= pow(1 + eps1, payoff[0][i][a2] / h);
        for(int i = 0; i < k; i++) w2[i] *= pow(1 + eps2, payoff[1][a1][i] / h);
        normalizeProbability(w1);
        normalizeProbability(w2);
    }
    ...
    {
        double winnings = 0.0;
        for(int j = 0; j < k; j++) winnings += payoff[0][i][j] * m2[j];
    ...
    {
        double winnings = 0.0;
        for(int j = 0; j < k; j++) winnings += payoff[1][j][i] * m1[j];
        fprintf(stderr,"Only action %d: %.0lf\n", i, winnings);
    }
}
